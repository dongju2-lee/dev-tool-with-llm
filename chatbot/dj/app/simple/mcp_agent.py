...
async def get_mcp_agent_async():
    """MCP 에이전트의 싱글톤 인스턴스를 비동기적으로 생성합니다."""
    global _agent_instance
    if _agent_instance is None:
        logger.info("MCP 에이전트 초기화 시작")
        
        # 모델 설정 가져오기
        model_name = os.environ.get("MCP_AGENT_MODEL", "gemini-2.0-flash")
        logger.info(f"MCP 에이전트 LLM 모델: {model_name}")
...
def get_mcp_agent():
    """MCP 에이전트의 싱글톤 인스턴스를 반환합니다."""
    global _agent_instance
    if _agent_instance is None:
        try:
            # 비동기 초기화 함수를 동기적으로 실행
            logger.info("MCP 에이전트 비동기 초기화 시작")
            loop = asyncio.get_event_loop()
            _agent_instance = loop.run_until_complete(get_mcp_agent_async())
            logger.info("MCP 에이전트 비동기 초기화 완료")
        except Exception as e:
            logger.error(f"MCP 에이전트 초기화 실패: {str(e)}")
            raise
...
async def mcp_agent_node(state: MessagesState) -> Command[Literal["supervisor"]]:
    ...
    try:
        # 에이전트 인스턴스 가져오기
        logger.info("MCP 에이전트 노드 함수 실행 시작")
        ...
        logger.error(f"MCP 에이전트 노드 함수 실행 중 오류 발생: {str(e)}")
