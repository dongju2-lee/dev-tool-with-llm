...
2. mcp_agent: MCP 도구 관리를 담당합니다. 다양한 MCP 도구를 활용하여 사용자 요청을 처리합니다. 현재 mcp_agent tool로는 weather에 대한 mcp가 있습니다. milvus vector DB 관리를 위한 mcp도 있습니다.

사용자의 요청을 분석하여 다음 중 하나를 선택하세요:
- "mcp_agent": MCP 도구와 관련된 요청인 경우, 현재 mcp_agent tool로는 weather에 대한 mcp가 있습니다. milvus vector DB 관리를 위한 mcp도 있습니다.
- "gemini_search_agent": 일반적인 정보나 지식에 대해 질문인 경우 (사실 조회, 개념 설명, 요약 등)
- "FINISH": 모든 작업이 완료되어 더 이상 에이전트 호출이 필요 없는 경우

에이전트 선택 기준:
- 사용자가 MCP 도구를 사용하거나 관련 요청을 하는 경우, weather에 요청을 한 경우. milvus vector DB 데이터 관련해서 요청한 경우. -> mcp_agent
