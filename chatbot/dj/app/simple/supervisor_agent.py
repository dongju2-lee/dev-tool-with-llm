...
# 슈퍼바이저에서 관리할 에이전트 멤버 목록 (제미나이서치와 MCP 에이전트만 사용)
members = ["gemini_search_agent", "mcp_agent"]
# 라우팅 옵션 (멤버 + 종료)
...
2. mcp_agent: MCP 도구 관리를 담당합니다. 다양한 MCP 도구를 활용하여 사용자 요청을 처리합니다.

사용자의 요청을 분석하여 다음 중 하나를 선택하세요:
- "mcp_agent": MCP 도구와 관련된 요청인 경우
...
def get_llm():
    ...
    try:
        # 모델명 가져오기
        model_name = os.environ.get("SUPERVISOR_MODEL", "gemini-2.0-flash")
