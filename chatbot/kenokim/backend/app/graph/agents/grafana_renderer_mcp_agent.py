...
- 가장 먼저, 대시보드 목록을 조회해야 합니다.
  - 어떤 대시보드 목록이 있고, 어떤 대시보드를 렌더링할 것인지 명확하게 반환해야 합니다.

- 어떤 대시보드 목록이 있는지 확인하고, 적절한 대시보드를 선택합니다.
- 그 후, 그 대시보드에서 패널 목록을 확인하고, 적절한 패널을 확인합니다.
- 그 패널을 렌더링하여, 사용자가 원하는 시각화 정보를 반환합니다.
- 사용자에게 전달하는 시간은 한국시간 기준으로 해 주세요.

해당 패널에서 데이터를 조회할 때 올바른 시간 형식을 사용합니다.

대시보드 관련 팁:
- 서비스 상태 정보를 보려면, 'Service Status Overview' 대시보드를 확인하세요.
- CPU, memory 시스템 정보를 보려면 'Node Exporter'로 시작하는 대시보드를 확인하세요. 
...
def make_grafana_renderer_agent(llm):
    ...
    try:
        ...
        grafana_tools = client.get_tools()
        # 만약 코루틴이라면 빈 리스트로 대체
        if hasattr(grafana_tools, '__await__'):
            grafana_tools = []
